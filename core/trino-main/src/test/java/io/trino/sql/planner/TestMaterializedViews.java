        ...
        extends BasePlanTest
{
    ...
    protected PlanTester createPlanTester()
    {
        ...
        createMaterializedView("simple_materialized_view", "SELECT a as new_name, b FROM test_table WHERE a is not null and b > 1", planTester);
        createMaterializedView("aggregation_materialized_view", "SELECT a, count(*) FROM test_table GROUP BY a", planTester);
        createMaterializedView("join_materialized_view", "SELECT a.a, b.b FROM test_table a JOIN test_table b on a.a = b.a", planTester);
        createMaterializedView("distinct_materialized_view", "SELECT distinct a, b FROM test_table", planTester);
        createMaterializedView("table_subquery_materialized_view", "SELECT a, b FROM (SELECT a, b FROM (VALUES (1, 2), (3, 4)) t(a, b))", planTester);
        createMaterializedView("where_subquery_materialized_view", "SELECT a, b FROM test_table WHERE b in (SELECT b FROM test_table WHERE a < 0)", planTester);
    ...
    private void createMaterializedView(String materializedViewName, String query, PlanTester planTester)
    {
        Metadata metadata = planTester.getPlannerContext().getMetadata();
    ...
    public void testRefreshTypes()
    {
        ...
        Assertions.assertThat(refreshType.get()).isEqualTo(RefreshType.INCREMENTAL);

        refreshType = getRefreshType("aggregation_materialized_view");
        Assertions.assertThat(refreshType.isPresent()).isTrue();
        Assertions.assertThat(refreshType.get()).isEqualTo(RefreshType.FULL);
