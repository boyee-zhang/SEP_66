        ...
        extends Codec
{
    ...
    private final Decompressor decompressor = new SnappyDecompressor();
            ...
            throws IOException
    {
        ...
        if (compressedData.limit() == compressedData.capacity() && compressedData.arrayOffset() == 0) {
            output = ByteBuffer.allocate(SnappyDecompressor.getUncompressedLength(compressedData.array(), compressedData.position()));
        }
        else {
            ...
            output = ByteBuffer.allocate(SnappyDecompressor.getUncompressedLength(input, 0));
        }
